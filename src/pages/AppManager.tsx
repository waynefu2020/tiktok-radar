...
import { getApps } from '../services/tikhub'
...
export default function AppManager() {
  ...
  return (
    <div className="p-6">
      <TopBar
        title="竞品管理"
        subtitle={`共 ${apps.length} 个竞品 · 在此添加或删除竞品 App`}
      />
      ...
      <div className="mt-6 grid grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border border-[#2E3045] bg-[rgb(12,14,20)] p-5">
          <h3 className="text-sm font-semibold text-white mb-4">添加竞品</h3>
          <form onSubmit={handleAdd} className="space-y-4">
            <div>
              <label className="block text-xs text-[#8A8FA8] mb-1.5">标识 ID（英文，如 astra）</label>
              <input
                value={form.id}
                onChange={e => setForm({ ...form, id: e.target.value })}
                placeholder="astra"
                className="w-full bg-[rgb(18,20,28)] border border-[#2E3045] rounded-lg px-3 py-2 text-sm text-[#C8CBE0] placeholder-[#555873] focus:outline-none focus:border-[#6366F1]/50"
              />
            </div>
            <div>
              <label className="block text-xs text-[#8A8FA8] mb-1.5">显示名称（如 AstraAI）</label>
              <input
                value={form.name}
            ...
            <div>
              <label className="block text-xs text-[#8A8FA8] mb-1.5">颜色</label>
            ...
            <div>
              <label className="block text-xs text-[#8A8FA8] mb-1.5">搜索关键词（多个用逗号分隔）</label>
              <input
                value={form.keywords}
        ...
        <div className="rounded-xl border border-[#2E3045] bg-[rgb(12,14,20)] p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-white">当前竞品</h3>
            <button onClick={loadApps} disabled={loading} className="text-[#8A8FA8] hover:text-[#C8CBE0] transition-colors">
          ...
          {loading && apps.length === 0 ? (
            ...
            <div className="space-y-3">
              {apps.map(app => (
                <div key={app.id} className="flex items-center gap-3 p-3 rounded-lg bg-[rgb(18,20,28)] border border-[#2E3045]">
                  <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: app.color }} />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-[#C8CBE0] font-medium">{app.name}</div>
                    <div className="text-[10px] text-[#555873]">ID: {app.id} · 关键词: {(app.keywords || []).join(', ')}</div>
                  </div>
                  <button
                    onClick={() => handleDelete(app.id)}
                    className="text-[#8A8FA8] hover:text-red-400 transition-colors"
                    title="删除"
                  >
                    <Trash2 size={14} />
                  </button>
